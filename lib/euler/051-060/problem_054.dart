...
class Hand {
  factory Hand(Iterable<String> input) {
    ...
    final cards = input.map(Card.new).toList(growable: false)..sort();
  ...
  int get rank {
    ...
    if (treeOfAKind.length == 3) {
      rank += 1e4 + 2e3 * treeOfAKind.last.value;
    }

    // Two Pairs: Two different pairs.
    if (twoOfAKind.length == 4) {
      rank += 1e3 + 2e2 * twoOfAKind.last.value;
    }

    // One Pair: Two cards of the same value.
    if (twoOfAKind.length == 2) {
      rank += 1e2 + 2e1 * twoOfAKind.last.value;
    }

    // High Card: Highest value card.
    rank += cards.last.value;
