...
final pattern = RegExp(r'(\d+)-(\d+) (\w): (\w+)');

bool isValid1(String value) {
  final match = pattern.matchAsPrefix(value)!;
  final min = int.parse(match.group(1)!);
  final max = int.parse(match.group(2)!);
  final letter = match.group(3)!;
  final password = match.group(4)!;
  final occurrences = password.toList().where((each) => each == letter).length;

  return min <= occurrences && occurrences <= max;
}

bool isValid2(String value) {
  final match = pattern.matchAsPrefix(value)!;
  final first = int.parse(match.group(1)!);
  final second = int.parse(match.group(2)!);
  final letter = match.group(3)!;
  final example = match.group(4)!;

  return (1 <= first &&
          first <= example.length &&
          example[first - 1] == letter) !=
      (1 <= second &&
          second <= example.length &&
          example[second - 1] == letter);
}
