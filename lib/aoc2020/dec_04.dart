...
final validation = <String, bool Function(String data)>{
  ...
  'byr': (data) {
    final year = int.tryParse(data);
    return year != null && 1920 <= year && year <= 2002;
  },
  // (Issue Year) - four digits; at least 2010 and at most 2020.
  'iyr': (data) {
    final year = int.tryParse(data);
    return year != null && 2010 <= year && year <= 2020;
  },
  // (Expiration Year) - four digits; at least 2020 and at most 2030.
  'eyr': (data) {
    final year = int.tryParse(data);
    return year != null && 2020 <= year && year <= 2030;
  },
  'hgt': (data) {
    ...
    if (match.group(2) == 'cm') {
      return height != null && 150 <= height && height <= 193;
    }
    // in, the number must be at least 59 and at most 76.
    if (match.group(2) == 'in') {
      return height != null && 59 <= height && height <= 76;
    }
    return false;
  },
...
void main() {
  var count = 0, valid = 0;
  for (final passport in passports.split('\n')) {
    final fields = <String, String>{};
    for (final field in passport.split(' ')) {
    ...
    final isPassport = validation.keys.every(fields.containsKey);
    if (isPassport) {
      count++;
      final isValid =
          validation.entries.every((each) => each.value(fields[each.key]!));
      if (isValid) {
        valid++;
      }
    }
