...
final String input = new String.fromCharCodes(inputRaw.split(',').map(int.parse));

final _a = 'a'.codeUnitAt(0);
final _z = 'z'.codeUnitAt(0);

String crypt(String input, String password) {
  return new String.fromCharCodes(new List.generate(input.length,
...
void main() {
  for (var a = _a; a <= _z; a++) {
    for (var b = _a; b <= _z; b++) {
      for (var c = _a; c <= _z; c++) {
        var output = crypt(input, new String.fromCharCodes([a, b, c]));
